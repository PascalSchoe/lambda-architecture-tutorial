...
docker build -t pschoe/hadoop:test .

#creating container from image
echo "starting container for master"
docker run -itd --network="hadoopnet" --ip 172.25.0.100 -p 50070:50070 -p 8088:8088 --name master --hostname master pschoe/hadoop:test

for (( j=1; j<=$nSlaves; j++ ))
do
	slavename="slave$j"
	echo "starting container for $slavename"
	docker run -itd --network="hadoopnet" --ip 172.25.0.10$j --name $slavename --hostname $slavename pschoe/hadoop:test
done

docker exec -ti master sh -c "hdfs namenode -format && /opt/hadoop/sbin/start-dfs.sh && /opt/hadoop/sbin/start-yarn.sh"
